...
def top_n_value(dataframe, agg_variable, filter_variable=None, value=None, n=5):
    try:
...
def relative_CumSumInfrequentValues(dataframe, column, n):
    ''' 
    This function takes n (integer) and prints the total number and percentage of 
    rows where values of column appear less than or equal to n times.
    '''
    value_counts = dataframe[column].value_counts() 
    infrequent_values = value_counts[value_counts <= n].index
    filtered_data = dataframe[dataframe[column].isin(infrequent_values)]
    percentage = filtered_data.shape[0] / dataframe.shape[0] * 100
    print(f'The total number of {column} values that appear less than or equal to {n} times is: {filtered_data.shape[0]}, which is {percentage:.3f}% of the total rows.')
...
def create_bins(dataframe, column, bins, labels, col_to_name):
    ...
    bins = bins
    labels = labels
    dataframe[col_to_name] = pd.cut(dataframe[column], bins = bins, labels = labels)
    return dataframe
